...
import { 
  generateYouTubeExcelTemplate, 
  processYouTubeImportData 
} from "./utils/youtubeUtils";
import { processExcelFile } from "./utils/commonUtils";

interface YouTubeImportTemplateProps {
  onSuccess?: () => void;
}

export function YouTubeImportTemplate({ onSuccess }: YouTubeImportTemplateProps) {
  ...
  const importMutation = useMutation({
    mutationFn: async (csvData: string) => {
      return processYouTubeImportData(
        csvData,
        setImportErrors,
        setImportSuccess
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2">
        ...
        <h2 className="text-xl font-bold">Plantilla: Usuario YouTube</h2>
      </div>
      
      <Card>
        <CardHeader>
          <CardTitle>Importar Creadores con YouTube</CardTitle>
          <CardDescription>
            Importa múltiples creadores con sus datos básicos y usuario de YouTube
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-gray-500">
              Formato: nombre, apellido, correo, usuario_youtube
            </p>
            <Button 
              variant="outline" 
              size="sm" 
          ...
          <Alert variant="default" className="bg-blue-50 text-blue-800 border-blue-200">
            <AlertDescription className="text-sm">
              La primera fila debe contener los encabezados: nombre,apellido,correo,usuario_youtube
            </AlertDescription>
          </Alert>
        </CardContent>
